...
class _HomePageState extends State<HomePage> {
  ...
  Future<void> fetchData() async {
    ...
    String url = AppUrl.intervention +
        '?demNumero=${widget.client!.resOppo['numero']}&dateDebut=${DateFormat('yyyy-MM-ddT00:00:00').format(AppUrl.filtredOpporunity.date)}&dateFin=${DateFormat('yyyy-MM-ddT23:59:59').format(AppUrl.filtredOpporunity.dateEnd)}&salCode=${collaborator}';

    if (AppUrl.filtredOpporunity.clinet != null &&
        AppUrl.filtredOpporunity.article == null) {
      url = AppUrl.intervention +
          '?demNumero=${widget.client!.resOppo['numero']}&dateDebut=${DateFormat('yyyy-MM-ddT00:00:00').format(AppUrl.filtredOpporunity.date)}&dateFin=${DateFormat('yyyy-MM-ddT23:59:59').format(AppUrl.filtredOpporunity.dateEnd)}&salCode=${collaborator}&pcfCode=${AppUrl.filtredOpporunity.clinet!.id}';
    }
    if (AppUrl.filtredOpporunity.clinet != null &&
        AppUrl.filtredOpporunity.article != null) {
      url = AppUrl.intervention +
          '?demNumero=${widget.client!.resOppo['numero']}&dateDebut=${DateFormat('yyyy-MM-ddT00:00:00').format(AppUrl.filtredOpporunity.date)}&dateFin=${DateFormat('yyyy-MM-ddT23:59:59').format(AppUrl.filtredOpporunity.dateEnd)}&salCode=${collaborator}&pcfCode=${AppUrl.filtredOpporunity.clinet!.id}&numSerie=${AppUrl.filtredOpporunity.article!.numSerie}';
    }
    print('url : $url');
    ...
    if (req.statusCode == 200) {
      ...
      for (var element in data.toList()) {
        ...
        if (req.statusCode == 200) {
          ...
          print('size of opp: ${provider.mapClientsWithCommandsInterventions.length}');
        }
      }
      provider.mapClientsWithCommandsInterventions
