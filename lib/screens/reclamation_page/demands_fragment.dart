...
import 'one_reclamation_page.dart';

class DemandsFragment extends StatefulWidget {
  final String? numSerie;

  const DemandsFragment({super.key, this.numSerie});

  @override
  State<DemandsFragment> createState() => _DemandsFragmentState();
}

class _DemandsFragmentState extends State<DemandsFragment> {
  Future<void> fetchData() async {
    ...
    String url = '';
    if (widget.numSerie != null) {
      url = AppUrl.reclamation + '?numSerie=${widget.numSerie}';
    } else {
    ...
    if (AppUrl.user.role == 'Administrateur') {
      if (widget.numSerie != null) {
        url = AppUrl.reclamation + '?numSerie=${widget.numSerie}';
      } else {
        url = AppUrl.reclamation +
        ...
        if (AppUrl.filtredOpporunity.clinet != null &&
            AppUrl.filtredOpporunity.article == null) {
          url = AppUrl.reclamation +
              '?dateDebut=${DateFormat('yyyy-MM-ddT00:00:00').format(AppUrl.filtredOpporunity.date)}&dateFin=${DateFormat('yyyy-MM-ddT23:59:59').format(AppUrl.filtredOpporunity.dateEnd)}&pcfCode=${AppUrl.filtredOpporunity.clinet!.id}';
        }
        if (AppUrl.filtredOpporunity.clinet != null &&
            AppUrl.filtredOpporunity.article != null) {
          url = AppUrl.reclamation +
              '?dateDebut=${DateFormat('yyyy-MM-ddT00:00:00').format(AppUrl.filtredOpporunity.date)}&dateFin=${DateFormat('yyyy-MM-ddT23:59:59').format(AppUrl.filtredOpporunity.dateEnd)}&pcfCode=${AppUrl.filtredOpporunity.clinet!.id}&numSerie=${AppUrl.filtredOpporunity.article!.numSerie}';
        }
      }
    }
    print('url: $url');
            ...
            if (clients.mapClientsWithCommands.length == 0)
              ...
              return ListView.builder(
                  ...
                  itemBuilder: (context, index) => InkWell(
                      ...
                      child: ClientItem(
                          client:
                              clients.mapClientsWithCommands.toList()[index])),
...
class _ClientItemState extends State<ClientItem> {
  ...
  Widget build(BuildContext context) {
    ...
    print('client is : ${widget.client.resOppo['tiers']['rs']}');
                        ...
                        Column(
                          ...
                          children: [
                            ...
                            (widget.client.resOppo['tiers']['rs'] != null)
                                ? Text(
                                    'Client : ${widget.client.resOppo['tiers']['rs']}',
                                    style: Theme.of(context)
                                        .textTheme
                                        .bodyText1!
                            ...
                            Text('Adresse : ${widget.client.adress}',
                                style: Theme.of(context)
                                    ...
                                    .copyWith(color: Colors.grey)),
                            Text(
                                'Équipement : ${widget.client.resOppo['article']['lib']}',
                                style: Theme.of(context)
                                    .textTheme
                                    .bodyText1!
                                    .copyWith(color: Colors.grey)),
