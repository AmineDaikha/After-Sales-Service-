...
class _StorePageState extends State<StorePage> {
  ...
  Future<void> fetchData() async {
    ...
    String url = AppUrl.articles +
        '?type=R,O&PageNumber=$PageNumber&Filter=$filter&PageSize=$PageSize';
    print('urlArticles: $url');
    ...
    if (req.statusCode == 200) {
      ...
      print('efefe : ${data.length}');
      data.toList().forEach((element) async {
        //if (element['categ'] == 'R' || element['categ'] == 'O') {
          print('code article:  ${element['code']}  ${element['categ']}');
          int? stock;
          double price = 0;
...
class _CommandItemState extends State<CommandItem> {
  ...
  Widget build(BuildContext context) {
    print(
        'dfdfev ${widget.product.quantity} zdzdz: ${widget.product.quantityOfColis}');
    int nbColis =
...
class StoreSearchDelegate extends SearchDelegate {
  ...
  Future<void> fetchData(BuildContext context) async {
    ...
    provider.filtredProducts = [];
    http.Response req = await http.get(
        Uri.parse(AppUrl.articles + '?type=R,O&PageNumber=1&Filter=$query&PageSize=20'),
        headers: {
          "Accept": "application/json",
          "content-type": "application/json; charset=UTF-8",
          "Referer": "http://" + AppUrl.user.company! + ".localhost:4200/"
