...
import 'package:sav_app/models/process.dart';
...
import 'package:sav_app/screens/home_page/activities_pages/activity_page.dart';
import 'package:sav_app/screens/home_page/one_intervention_page.dart';
import 'package:sav_app/screens/home_page/opportunity_page.dart';
...
class _NotifsListPageState extends State<NotifsListPage> {
  ...
  Future<void> fetchDataNotif() async {
    ...
    print("res notifs code : ${req.statusCode}");
    print("res notifs body: ${req.body}");
...
class _NotifItemState extends State<NotifItem> {
  ...
  Future<Client?> getInterv(String code) async {
    String url = AppUrl.intervention + '/${code}';
    print('oneInterv $url');
    print('url : $url');
    http.Response req = await http.get(Uri.parse(url), headers: {
      "Accept": "application/json",
    ...
    if (req.statusCode == 200) {
      ...
      var element = json.decode(req.body);
        ...
        if (req.statusCode == 200) {
          ...
          print('resOpp:: ${client.resOppo['demande']}');
        return client;
        }
    return null;
  }}
    ...
    return InkWell(
      onTap: () {
        ...
        changeToSeen(widget.notif).then((value) {
          if (value) {
            setState(() {});
            if (widget.notif.type == 'O' || widget.notif.type == 'I') {
              getInterv(widget.notif.codeLie.toString()).then((client) {
                if (client != null) {
                  PageNavigator(ctx: context)
                      .nextPage(page: OneInterventionPage(client: client)).then((value) => Navigator.pop(context));
                } else {
                  showAlertDialog(
