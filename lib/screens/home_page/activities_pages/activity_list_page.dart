...
import 'package:sav_app/screens/activities_pages/dialog_cancel_activities.dart';
...
import 'package:sav_app/widgets/dialog_filtred_activities.dart';
...
class _ActivityListPageState extends State<ActivityListPage> {
  ...
  Future<void> fetchData(BuildContext context) async {
    ...
    String url = AppUrl.getAcivities + '?interNumero=${widget.client.idOpp}';
    print('urlActOpp: $url');
    ...
    if (req.statusCode == 200) {
      ...
      try {
        ...
        for (int i = 0; i < data.length; i++) {
          ...
          if(activity.state == states[0] && duration.inDays > AppUrl.dayDepasAct && AppUrl.dayDepasAct > 0){
            activity.state = states[3];
            await unterminedActivity(activity);
          }
          if(activity.state == states[1] && duration.inDays > AppUrl.dayDepasCoursAct && AppUrl.dayDepasCoursAct > 0){
            activity.state = states[3];
            await unterminedActivity(activity);
          }
  ...
  //                       showLoaderDialog(context);
  //                       dataItem.state = states[4];
  //                       cancelActivity(context, dataItem)
  //                           .then((value) {
  //                         if (value) {
  //                           showMessage(
  //                               message:
  //                               'Activité annulée avec succès',
  //                               context: context,
  //                               color: primaryColor);
  //                           Navigator.pop(context);
  //                           setState(() {});
  ...
  //                   ? Text(dataItem.start!.split(' ')[0])
  //                   : Text('')),
  //               DataCell((dataItem.dateStart != null)
  //                   ? Text(dataItem.start!.split(' ')[1])
  //                   : Text('')),
  //               DataCell((dataItem.dateEnd != null)
  //                   ? Text(dataItem.end!.split(' ')[0])
  //                   : Text('')),
  //               DataCell((dataItem.dateEnd != null)
  //                   ? Text(dataItem.end!.split(' ')[1])
  //                   : Text('')),
  //               DataCell(Text('')),
  //               DataCell(Text(dataItem.contactTxt!)),
  //               DataCell((dataItem.state != null)
