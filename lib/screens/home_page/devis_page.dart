...
import 'package:sav_app/screens/catalog_page/store_page.dart';
...
class _DevisPageState extends State<DevisPage> {
  ...
  Future<void> fetchPaidData() async {
    ...
    if (req.statusCode == 200) {
      ...
      data.toList().forEach((element) {
        print(
            'cmpar : ${widget.client.command!.id}  ${element['docNumero']}  ${widget.client.command!.date}');
        try {
                      ...
                      child: Row(
                        ...
                        children: [
                          ...
                          Column(
                            ...
                            children: [
                              Text(
                                'Date : ${DateFormat('yyyy-MM-dd HH:mm:ss').format(widget.client.command!.date!)}',
                                style: Theme.of(context)
                                    .textTheme
                                    .headline5!
                                    .copyWith(
                                  ...
                                  _getCurrentLocation().then((value) {
                                    if (value) {
                                      toCommand(widget.client)
                                        ...
                                        if (editCommandValue) {
                                          _showAlertConfirmationDialog(context,
  ...
  Future<String> verifyStock() async {
    for (Product product in widget.client.command!.products) {
      print('efeffee : ${product.typeStock}');
      if (product.typeStock == 'M') {
...
class _CommandItemState extends State<CommandItem> {
  ...
  Future<void> getQuantityMax(Product product) async {
    ...
    await getTypeStock(product);
  }

  Future<void> getTypeStock(Product product) async {
    first = false;
    String url = AppUrl.articles + '/${product.id}';
    print('urlgetOneArticle: $url');
    http.Response req = await http.get(Uri.parse(url), headers: {
      "Accept": "application/json",
      "content-type": "application/json; charset=UTF-8",
      "Referer": "http://" + AppUrl.user.company! + ".localhost:4200/",
    });
    print("res getOneArt code : ${req.statusCode}");
    print("res getOneArt body: ${req.body}");
    if (req.statusCode == 200) {
      var res = json.decode(req.body);
      String qt = res['stock'];
      product.typeStock = qt;
      print('frrgrgr $qt');
    } else
      product.typeStock = null;
  }
